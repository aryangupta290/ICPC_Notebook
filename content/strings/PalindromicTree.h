...
};struct eertree{int rto,rte,n,last;
...
if(flag){int idx=SZ(tree);tree.PB(node());
tree[idx].len=tree[last].len+2;
if(tree[idx].len==1){
tree[idx].suf_lnk=rte;}else{
tree[idx].suf_lnk=tree[tree[last].smt_lnk[c]].next[c];
}if(tree[idx].len==1){
REP(cc,0,SIGMA){tree[idx].smt_lnk[cc]=0;}
tree[idx].smt_lnk[c]=1;}else{
REP(cc,0,SIGMA){int x=tree[idx].suf_lnk;
if(s[n-tree[x].len]==cc)
tree[idx].smt_lnk[cc]=x;else
