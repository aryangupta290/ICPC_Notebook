...
edges.emplace_back();REP(i,0,SZ(s)){
edges.emplace_back();
length.push_back(i+1);link.push_back(0);
int r=SZ(edges)-1,p=last;
while(p>=0&&edges[p].find(s[i])==edges[p].end()){
edges[p][s[i]]=r,p=link[p];}if(p!=-1){
const int q=edges[p][s[i]];
if(length[p]+1==length[q])link[r]=q;else{
...
edges[p][s[i]]=qq;}}last=r;}VI terminals;
