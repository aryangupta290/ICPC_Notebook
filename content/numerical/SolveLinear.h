...
double v,bv=0;REP(r,i,n)REP(c,i,m)
if((v=fabs(A[r][c]))>bv)br=r,bc=c,bv=v;
if(bv<=eps){
REP(j,i,n)if(fabs(b[j])>eps)return-1;
break;}swap(A[i],A[br]);swap(b[i],b[br]);
swap(col[i],col[bc]);
REP(j,0,n)swap(A[j][i],A[j][bc]);
bv=1/A[i][i];REP(j,i+1,n){
double fac=A[j][i]*bv;b[j]-=fac*b[i];
REP(k,i+1,m)A[j][k]-=fac*A[i][k];}rank++;}
