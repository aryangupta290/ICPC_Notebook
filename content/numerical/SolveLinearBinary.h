...
VI col(m);iota(ALL(col),0);REP(i,0,n){
for(br=i;br<n;++br)if(A[br].any())break;
if(br==n){REP(j,i,n)if(b[j])return-1;
break;}int bc=(int)A[br]._Find_next(i-1);
swap(A[i],A[br]);swap(b[i],b[br]);
swap(col[i],col[bc]);
REP(j,0,n)if(A[j][i]!=A[j][bc]){
A[j].flip(i);A[j].flip(bc);}
REP(j,i+1,n)if(A[j][i]){b[j]^=b[i];
A[j]^=A[i];}rank++;}x=bs();
for(int i=rank;i--;){if(!b[i])continue;
x[col[i]]=1;REP(j,0,i)b[j]^=A[j][i];}
