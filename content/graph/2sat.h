...
two_sat(){}two_sat(int_n){init(_n);}
void init(int_n){n=_n;g.assign(2*n,VI());
gr.assign(2*n,VI());comp.resize(2*n);
vis.resize(2*n);ans.resize(2*n);}
void add(int u,int v){
g[u].PB(v);gr[v].PB(u);}
/*For the following three functions int x,bool val:if'val'is true,we take the variable to be x.Otherwise we take it to be x's complement. At least one of them is true*/
...
add_xor(i,!f,j,g);}//Topological sort
