...
typedef vector<ll>VL;struct MCMF{
...
red[to].push_back(from);}void path(int s){
...
ll val=di-pi[i]+cost;if(cap&&val<dist[i]){
dist[i]=val;par[i]={s,dir};
...
while(path(s),seen[t]){ll fl=INF;
for(int p,r,x=t;tie(p,r)=par[x],x!=s;x=p)
fl=min(fl,r?cap[p][x]-flow[p][x]:flow[x][p]);
totflow+=fl;
for(int p,r,x=t;tie(p,r)=par[x],x!=s;x=p)
if(r)flow[p][x]+=fl;else flow[x][p]-=fl;}
REP(i,0,N)REP(j,0,N)totcost+=cost[i][j]*flow[i][j];
return{totflow,totcost};}
//If some costs can be negative,call this before maxflow:
void setpi(int s){//(otherwise,leave this out)
fill(ALL(pi),INF);pi[s]=0;
int it=N,ch=1;ll v;while(ch--&&it--)
