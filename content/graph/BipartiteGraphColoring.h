 ...
 * License: CC0
 * Source: Folklore
 * Description: change max-values of m and n according to problem.
...
#pragma once
struct edge_color{int deg[2][MAXN];
II has[2][MAXN][MAXN];int color[MAXM];
int c[2];void clear(int n){
for(int t=0;t<2;t++){
...
for(int i=1;i<=m;i++){int x[2];
x[0]=v[i-1].first;x[1]=v[i-1].second;
for(int d=0;d<2;d++){deg[d][x[d]]+=1;
